...
export default function Home() {
  ...
  // Reveal sections after entering
  useEffect(() => {
    if (!entered) return
    const delays = [200, 600, 1200, 1800, 3000, 4200, 5200, 6200]
  ...
  // Handle splash enter - this is the ONLY entry point for audio
  const handleEnter = () => {
    if (entered) return
    setEntered(true)
    // Unlock body scroll
    ...
    // Start audio - this works because it's inside a user gesture (click/touch)
    if (audioRef.current) {
      ...
      if (playPromise) {
        playPromise.catch(() => {
          // Retry with volume trick for some Android browsers
          if (audioRef.current) {
  ...
  return (
    <>
      ...
      {!entered && (
        ...
        >
          ...
          <div className="splash-content">
            ...
            <p className="splash-title">Feliz Cumpleaños</p>
            <p className="splash-date">13 / 06 / 26</p>
            <div className="splash-tap-area">
              <div className="splash-tap-ring" />
              <span className="splash-tap-text">Toca para abrir</span>
            </div>
          </div>
        </div>
      )}
      ...
      {/* ===== MAIN LANDING ===== */}
      <div
        className={`landing-page ${entered ? 'visible' : 'hidden-page'}`}
      >
        {/* Flash overlay */}
      ...
      <style jsx global>{`
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .splash-date {
          font-family: 'Playfair Display', Georgia, serif;
          font-size: 1.1rem; color: rgba(212, 175, 55, 0.6);
        ...
        .splash-tap-ring {
          ...
          border-radius: 50%;
          animation: tapRing 2s ease-in-out infinite;
        }

        @keyframes tapRing {
          0% { transform: scale(0.8); opacity: 0.3; border-color: rgba(212, 175, 55, 0.1); }
          50% { transform: scale(1.2); opacity: 1; border-color: rgba(212, 175, 55, 0.6); }
          100% { transform: scale(0.8); opacity: 0.3; border-color: rgba(212, 175, 55, 0.1); }
        }

        .splash-tap-text {
          font-family: 'Inter', sans-serif;
          font-size: 0.9rem; color: rgba(212, 175, 55, 0.7);
        ...
        .landing-page {
          ...
          color: #f0e6d3; padding: 2rem 1rem;
        ...
        .date-section.revealed { opacity: 1; transform: translateY(0) scale(1); }
        ...
        .date-inner { position: relative; display: flex; align-items: center; justify-content: center; gap: 2px; }
        .date-digit {
        ...
        .date-sep {
          ...
          color: rgba(212, 175, 55, 0.5); margin: 0 4px;
          animation: digitPop 0.4s cubic-bezier(0.16, 1, 0.3, 1) 0.3s both;
        }
        .date-sparkles { display: flex; justify-content: center; gap: 1.5rem; margin-top: 0.3rem; }
        .date-sparkle { font-size: 0.5rem; color: #d4af37; animation: sparkle 2s ease-in-out infinite; opacity: 0.5; }
        ...
        .header-section.revealed { opacity: 1; transform: translateY(0); }
        ...
        .letter-decoration, .letter-greeting, .letter-divider, .letter-text, .letter-signature {
          opacity: 0; transform: translateY(15px); transition: all 1s cubic-bezier(0.16, 1, 0.3, 1);
          max-height: 0; overflow: hidden; margin-bottom: 0;
        }
        .letter-decoration.revealed, .letter-greeting.revealed, .letter-divider.revealed,
        .letter-text.revealed, .letter-signature.revealed {
          opacity: 1; transform: translateY(0); max-height: 500px; overflow: visible;
        }
        .letter-text.revealed { margin-bottom: 1rem; }

        .letter-decoration { text-align: center; color: #d4af37; letter-spacing: 1em; font-size: 0.7rem; margin: 0; transition: all 0.8s cubic-bezier(0.16, 1, 0.3, 1); }
        .letter-decoration.revealed { opacity: 0.6; margin: 0.5rem 0; }
        .letter-greeting { font-family: 'Playfair Display', Georgia, serif; font-size: 1.3rem; font-weight: 600; color: #d4af37; text-align: center; }
        ...
        .letter-text { font-size: 0.95rem; line-height: 1.7; color: rgba(240, 230, 211, 0.9); text-align: justify; font-weight: 300; }
        .letter-signature { text-align: center; font-family: 'Playfair Display', Georgia, serif; font-size: 1.1rem; line-height: 1.8; color: rgba(240, 230, 211, 0.95); }
        .signature-highlight { color: #d4af37; font-weight: 600; font-size: 1.15rem; }

        /* ===== HEART SECTION ===== */
        .heart-section {
          position: relative; display: flex; flex-direction: column; align-items: center; padding: 2rem 0;
          opacity: 0; transform: scale(0.5); transition: all 1.2s cubic-bezier(0.16, 1, 0.3, 1);
        }
        .heart-section.revealed { opacity: 1; transform: scale(1); }
        .heart-section.heart-exploded { transform: scale(0.8); opacity: 0; pointer-events: none; }
